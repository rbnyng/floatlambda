...
fn native_conv2d(vm: &mut VM) -> Result<(), InterpretError> {
    let padding = vm.pop_stack()? as usize;
    let stride = vm.pop_stack()? as usize;
    let bias_ptr = vm.pop_stack()?;
    let weights_ptr = vm.pop_stack()?;
    let input_ptr = vm.pop_stack()?;

    let bias_id = decode_heap_pointer(bias_ptr).ok_or_else(|| InterpretError::Runtime("conv2d expects a tensor for bias".to_string()))?;
    let weights_id = decode_heap_pointer(weights_ptr).ok_or_else(|| InterpretError::Runtime("conv2d expects a tensor for weights".to_string()))?;
    let input_id = decode_heap_pointer(input_ptr).ok_or_else(|| InterpretError::Runtime("conv2d expects a tensor for input".to_string()))?;

    let bias = vm.heap.get_tensor_mut(bias_id).map_err(|e| InterpretError::Runtime(e.to_string()))?.clone();
    let weights = vm.heap.get_tensor_mut(weights_id).map_err(|e| InterpretError::Runtime(e.to_string()))?.clone();
    let input = vm.heap.get_tensor_mut(input_id).map_err(|e| InterpretError::Runtime(e.to_string()))?.clone();
...
fn native_max_pool2d(vm: &mut VM) -> Result<(), InterpretError> {
    let stride = vm.pop_stack()? as usize;
    let kernel_size = vm.pop_stack()? as usize;
    let input_ptr = vm.pop_stack()?;

    let input_id = decode_heap_pointer(input_ptr).ok_or_else(|| InterpretError::Runtime("max_pool2d expects a tensor for input".to_string()))?;
    let input = vm.heap.get_tensor_mut(input_id).map_err(|e| InterpretError::Runtime(e.to_string()))?.clone();
...
fn native_tensor(vm: &mut VM) -> Result<(), InterpretError> {
    ...
    let shape_vec = list_to_vec(shape_list_ptr, vm.heap).map_err(|e| InterpretError::Runtime(e.to_string()))?.iter().map(|&x| x as usize).collect();
    let data_vec = list_to_vec(data_list_ptr, vm.heap).map_err(|e| InterpretError::Runtime(e.to_string()))?;
...
fn native_adamw_update(vm: &mut VM) -> Result<(), InterpretError> {
    ...
    let (new_params, new_m, new_v, new_t) = ml::optimizers::adamw_update(&params, &grads, &m, &v, t, lr, beta1, beta2, epsilon, weight_decay).map_err(|e| InterpretError::Runtime(e.to_string()))?;
...
fn native_reshape(vm: &mut VM) -> Result<(), InterpretError> {
    ...
    let new_shape_vec = list_to_vec(new_shape_ptr, vm.heap).map_err(|e| InterpretError::Runtime(e.to_string()))?.iter().map(|&x| x as usize).collect();
...
fn native_grad(vm: &mut VM) -> Result<(), InterpretError> {
    ...
    let input_tensor_id = decode_heap_pointer(input_tensor_ptr).ok_or_else(|| InterpretError::Runtime("grad expects a tensor as the second argument".to_string()))?;
    vm.stack.push(func_ptr);
    vm.stack.push(input_tensor_ptr);
    vm.call_value(func_ptr, 1)?;
    let output_tensor_ptr = vm.run()?;
    vm.pop_stack()?;
    let output_tensor_id = decode_heap_pointer(output_tensor_ptr).ok_or_else(|| InterpretError::Runtime("grad function must return a tensor.".to_string()))?;
    {
        ...
        if output_tensor.data.len() != 1 {
            return Err(InterpretError::Runtime(format!("grad function must return a scalar tensor (length 1), but got shape {:?}", output_tensor.shape)));
        }
...
pub const ML_NATIVES: &[NativeDef] = &[
    ...
    NativeDef { name: "conv2d", arity: 5, func: native_conv2d },
    NativeDef { name: "max_pool2d", arity: 3, func: native_max_pool2d },
