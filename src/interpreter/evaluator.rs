...
pub fn apply_function(func_val: f64, arg_val: f64, heap: &mut Heap) -> Result<f64, EvalError> {
    ...
    if let Some(id) = decode_heap_pointer(func_val) {
        ...
        match heap_obj {
            ...
            HeapObject::Upvalue(_) => Err(EvalError::TypeError("Cannot apply a VM closure in the tree-walker.".to_string())),
        }
    } else {
                    ...
                    Continuation::AppArg { func_val } => {
                        ...
                        if let Some(id) = decode_heap_pointer(func_val) {
                            ...
                            match heap_obj {
                                ...
                                HeapObject::Upvalue(_) => return Err(EvalError::TypeError("Cannot apply a VM closure in the tree-walker.".to_string())),
                            }
